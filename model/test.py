...
class UresNet(nn.Module):
    def __init__(self, in_channels, n_labels,block_cfg=[3,4,6,3]):
...
class ResUNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        long_range2 = F.dropout(long_range2, 0.2, self.training)

        short_range2 = self.down_conv2(long_range2)

        long_range3 = self.encoder_stage3(short_range2) + short_range2
        long_range3 = F.dropout(long_range3, 0.2, self.training)

        short_range3 = self.down_conv3(long_range3)

        long_range4 = self.encoder_stage4(short_range3) + short_range3
        long_range4 = F.dropout(long_range4, 0.2, self.training)

        short_range4 = self.down_conv4(long_range4)

        outputs = self.decoder_stage1(long_range4) + short_range4
        outputs = F.dropout(outputs, 0.2, self.training)
...
net = ResUNet(training=True).to("cuda")
net.apply(init)
